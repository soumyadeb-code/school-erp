...
@section('title', 'Bill Layout Designer')

@section('page-title', 'Bill Layout Designer')
        ...
        <div class="card">
            <div class="card-header">
                <div class="d-flex justify-content-between align-items-center">
                    <h5 class="mb-0">Configure Bill Layout - {{ ucfirst($billType) }} Bill</h5>
                    <div>
                        <a href="{{ route('school-admin.bill-layouts.designer', ['bill_type' => 'admission']) }}" 
                           class="btn btn-sm {{ $billType == 'admission' ? 'btn-primary' : 'btn-outline-primary' }}">
                            Admission
                        </a>
                        <a href="{{ route('school-admin.bill-layouts.designer', ['bill_type' => 'registration']) }}" 
                           class="btn btn-sm {{ $billType == 'registration' ? 'btn-primary' : 'btn-outline-primary' }}">
                            Registration
                        </a>
                        <a href="{{ route('school-admin.bill-layouts.designer', ['bill_type' => 'monthly']) }}" 
                           class="btn btn-sm {{ $billType == 'monthly' ? 'btn-primary' : 'btn-outline-primary' }}">
                            Monthly
                        </a>
                    </div>
                </div>
            ...
            <div class="card-body">
                ...
                <div class="alert alert-info mb-3">
                    <div class="d-flex align-items-center">
                        <div class="flex-grow-1">
                            ...
                            <ol class="mb-0 mt-2">
                                <li>Upload a blank PDF template for the {{ $billType }} bill</li>
                ...
                <!-- Upload Form -->
                <form action="{{ route('school-admin.bill-layouts.upload') }}" method="POST" enctype="multipart/form-data" class="mb-3" id="uploadForm">
                    @csrf
                ...
                <div class="alert alert-success">
                    ...
                    <strong>Template Loaded:</strong> {{ $templatePath }}
                </div>
                @endif
                ...
                <div class="designer-container" style="position: relative; overflow: auto; background: #525659; padding: 20px; min-height: 500px;">
                    ...
                    <div id="pdfContainer" style="position: relative; display: inline-block; box-shadow: 0 4px 8px rgba(0,0,0,0.3);">
                        @if(file_exists(public_path($templatePath)))
                        <iframe id="pdfFrame" src="{{ asset($templatePath) }}#toolbar=0&navpanes=0&scrollbar=0" 
                    ...
                    <div class="mt-3 p-3 bg-dark rounded">
                        ...
                        <div class="d-flex flex-wrap gap-2" id="fieldPalette">
                            ...
                            <div class="draggable-field badge bg-primary fs-6 p-2" 
                                 ...
                                 data-x="{{ $var['x'] }}"
                                 data-y="{{ $var['y'] }}"
                                 data-font-size="{{ $var['font_size'] }}"
                                 style="cursor: grab; pointer-events: auto;">
                                <i class="fas fa-grip-lines me-1"></i>{{ $var['label'] }}
                            </div>
                            @endforeach
                        </div>
                    </div>
                ...
                <form action="{{ route('school-admin.bill-layouts.store') }}" method="POST" id="layoutForm">
                    @csrf
                    <input type="hidden" name="bill_type" value="{{ $billType }}">
                ...
                <div class="mt-3">
                    ...
                    <a href="{{ route('school-admin.bill-layouts.reset', ['bill_type' => $billType]) }}" 
...
fieldPositions['{{ $key }}'] = {
    ...
    placed: {{ $var['x'] > 0 || $var['y'] > 0 ? 'true' : 'false' }}
};
@endforeach
